...
export class PayrollEventConsumer implements IEventConsumer {
    async consumeEvent(event: string, payload: PayRollEventSchemaType[]): Promise<any> {
        try {
            ...
            console.log("reached");
            const new_payroll: PayrollSchemaType = await this.calculatePayroll(payload) as PayrollSchemaType;
    ...
    calculatePayroll = async (payload: PayRollEventSchemaType[]): Promise<PayrollSchemaType> => {
        try {
            ...
            const summary: PayrollSchemaType = {
